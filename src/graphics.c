...
void start() {
	...
	draw_number((conf.time_ms / 100) % 60, w_s, w_s + 6); // Вывод секунд
	draw_number((conf.time_ms / 6000) % 60, w_m, w_m + 6); // Вывод минут
	draw_number((conf.time_ms / 360000), w_h, w_h + 6); // Вывод часов

	draw_number((conf.time_ms / 100) % 60, b_s, b_s + 6);
	draw_number((conf.time_ms / 6000) % 60, b_m, b_m + 6);
	draw_number((conf.time_ms / 360000), b_h, b_h + 6);
	ms_delay(1000);
